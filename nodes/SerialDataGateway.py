...
class SerialDataGateway(object):
	...
	def __init__(self, port="/dev/ttyUSB1", baudrate=19200, lineHandler = _OnLineReceived):
		'''
	...
	def Start(self):
		...
		self._ReceiverThread = threading.Thread(target=self._Listen).start()
	...
	def Write(self, data):
		#info = "Writing to serial port: %s"%data.encode('hex')
		#rospy.loginfo(info)
